...
public Action Command_MutePlayer(int client, int args)
{
	...
	if (!g_hAllowVipMute.BoolValue)
	{
		ReplyToCommand(client, "[%c%s%c] VIP muting has been disabled on IG temporarily.", MOSSGREEN, g_szChatPrefix, WHITE);
		return Plugin_Handled;
	}
	...
	if (args > 0)
	{
		...
		if (target != -1)
		{
			...
			{
				if (SourceComms_SetClientMute(target, false))
					PrintToChatAll("[%c%s%c] %s was unmuted by a VIP, %N.", MOSSGREEN, g_szChatPrefix, WHITE, szName, client);
			}
			else
			{
				if (SourceComms_SetClientMute(target, true, -1, true, "Muted by VIP"))
					PrintToChatAll("[%c%s%c] %s was muted by a VIP, %N.", MOSSGREEN, g_szChatPrefix, WHITE, szName, client);
			}
			return Plugin_Handled;
		}
		else
	...
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			if (IsValidClient(clientID))
			{
				...
				{
					if (SourceComms_SetClientMute(clientID, false))
						PrintToChatAll("[%c%s%c] %s was unmuted by a VIP, %N.", MOSSGREEN, g_szChatPrefix, WHITE, szName, client);
				}
				else
				{
					if (SourceComms_SetClientMute(clientID, true, -1, true, "Muted by VIP"))
						PrintToChatAll("[%c%s%c] %s was muted by a VIP, %N.", MOSSGREEN, g_szChatPrefix, WHITE, szName, client);
